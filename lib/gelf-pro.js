...
gelf.getAdapter = function () {
  if (!this.adapter) {
    var adapters = {
        udp: require('./adapter/udp'),
        tcp: require('./adapter/tcp'),
        'tcp-tls': require('./adapter/tcp-tls'),
    };
    // In react-native dynamic loading is not supported
    // because of the way packager works, so instead we
    // require all three possibilites and put them in a
    // object that we can index.
    this.adapter = adapters[this.config.adapterName];
