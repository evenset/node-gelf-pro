...
gelf.getAdapter = function () {
  if (!this.adapter) {
    // This always go UDP - Update it to read the adapter dynamically
    // from this.config.adapterName - In react-native dynamic loading 
    // is not supported because of the way packager works. 
    this.adapter = require('./adapter/udp');
